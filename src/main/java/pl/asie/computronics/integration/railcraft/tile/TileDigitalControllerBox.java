...
public class TileDigitalControllerBox extends TileDigitalBoxBase implements IControllerTile {
    ...
    private Object[] getAspect(String name) {
        ...
        Collection<WorldCoordinate> coords = Sets.newHashSet(this.controller.getCoordsFor(name));
        if (!coords.isEmpty()) {
            for (WorldCoordinate coord : coords) {
                aspect = this.controller.getAspectFor(coord);
            }
        }
    ...
    @Callback(doc = "function(name:string):boolean; Tries to get the aspect for the receiver with the specified name. Returns Aspect on success.", direct = true, limit = 32)
    @Optional.Method(modid = Mods.OpenComputers)
